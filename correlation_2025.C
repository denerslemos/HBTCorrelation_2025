...
*/
void correlation_2025(TString input_file, TString ouputfile, int isMC, int doquicktest, int domixing, int Nmixevents, int mincentormult, float minvz, int hbt3d, int gamov, int cent_bool, int syst){
	...
	*/
	...
	for(int i = 0; i < nevents; i++){
		...
		// Apply event filters (not yet available)
		/* 
		...
		*/
		...
		for (int j = 0; j < ntrk; j++){ 
		    ...
		    // Track QA array
			double x_reco_trk[5]={trkpt->at(j),trketa->at(j),trkphi->at(j),(double) trkcharge->at(j),(double) (use_centrality ? cent : Ntroff)}; 

			// Define some quantities		
			int NLayer = (int) trknlayer->at(j);
			int NHits = (int) trknhits->at(j);
			int Npixelhit = (int) trkpixhits->at(j);
			...
			if(trkpt->at(j) <= 0.2) continue;
			if(fabs(trketa->at(j)) > 2.4) continue;
			if(highpur->at(j) == false) continue;
			if(syst != 8) if (Npixelhit == 0) continue;

			if(syst == 3){
				if(fabs(trkpterr->at(j)/trkpt->at(j)) >= 0.05) continue;
				if(fabs(trkdcaxy->at(j)/trkdcaxyerr->at(j)) >= 2.0) continue;
				if(fabs(trkdcaz->at(j)/trkdcazerr->at(j)) >= 2.0) continue;
			}else if(syst == 4){
				if(fabs(trkpterr->at(j)/trkpt->at(j)) >= 0.1) continue;
				if(fabs(trkdcaxy->at(j)/trkdcaxyerr->at(j)) >= 5.0) continue;
				if(fabs(trkdcaz->at(j)/trkdcazerr->at(j)) >= 5.0) continue;
			}else{
				if(fabs(trkpterr->at(j)/trkpt->at(j)) >= 0.1) continue;
				if(fabs(trkdcaxy->at(j)/trkdcaxyerr->at(j)) >= 3.0) continue;
				if(fabs(trkdcaz->at(j)/trkdcazerr->at(j)) >= 3.0) continue;
			}

			// store quantities with selection
			ptresolution->Fill(fabs(trkpterr->at(j)/trkpt->at(j)));
			dxyoversigmadxy->Fill((trkdcaxy->at(j)/trkdcaxyerr->at(j)));
			dzoversigmadz->Fill((trkdcaz->at(j)/trkdcazerr->at(j)));
			chi2overNDFoverNLayer->Fill(trknormchi2->at(j)/(float)NLayer);
			...
			//trk_weight = trk_weight*getTrkCorrWeight(fileeff, cent, trkpt->at(j), trketa->at(j));
			hist_reco_trk_corr->Fill(x_reco_trk,trk_weight);
			
		    ROOT::Math::PtEtaPhiMVector TrackFourVector;
      		TrackFourVector.SetM(pi_mass);
      		TrackFourVector.SetEta(trketa->at(j));
     		TrackFourVector.SetPhi(trkphi->at(j));
     		TrackFourVector.SetPt(trkpt->at(j));  
